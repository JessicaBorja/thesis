...
class AffordancePixelModule(LightningModule):
    ...
    def attn_step(self, frame, label, compute_err=False):
        ...
        out_aff = self.attn_forward(inp, softmax=False)

        out_depth = self.depth(out_aff)
        out = {"aff": out_aff, "depth": out_depth} 

        attn_loss = self.attn_criterion(compute_err, inp, out, label)
        return attn_loss
