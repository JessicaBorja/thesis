...
from thesis.affordance.core.affordance_pixel_depth_module import AffordancePixelModule
...
class AffLangDetector(AffordancePixelModule):
    ...
    def _build_model(self):
        ...
        self.depth_mu = nn.Linear()
        self.depth_sigma = nn.Linear()

    def forward(self, inp, reparametrize=False):
        ''' 
            Depth compute forward pass
        '''
        mu = self.depth_mu(inp)
        sigma = self.depth_sigma(inp)
